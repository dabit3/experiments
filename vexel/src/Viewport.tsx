...
export function Viewport(props: Props) {
  ...
  useEffect(() => {
    ...
    catch { setError('WebGL is unavailable. Enable hardware acceleration and reload.'); return }
    ...
    renderer.domElement.setAttribute('aria-label', 'Four synchronized 3D viewports. Click objects to select; drag Perspective to orbit; scroll to zoom.')
    ...
    room.dispose()
    pmrem.dispose()
    const ambient = new THREE.HemisphereLight('#d8e5e4', '#8e7761', 2.2)
    scene.add(ambient)
    const sun = new THREE.DirectionalLight('#fff0d1', 4.1)
    ...
    scene.add(sun)
    const fill = new THREE.DirectionalLight('#e9f1ff', 1.6)
    ...
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), new THREE.MeshStandardMaterial({ color: '#797f76', roughness: 1 }))
    ...
    const top = ortho(), front = ortho(), left = ortho()
    ...
    const resetCamera = () => {
      perspective.position.set(13.5, 9, 19)
    ...
    const getRect = (index: number) => {
      ...
      return { x: (index % 2) * width / 2, y: Math.floor(index / 2) * height / 2, width: width / 2, height: height / 2 }
    }
    ...
    const up = (event: PointerEvent) => {
      ...
      const index = indexAt(event)
      const rect = getRect(index), bounds = el.getBoundingClientRect()
      mouse.set((event.clientX - bounds.left - rect.x) / rect.width * 2 - 1, -(event.clientY - bounds.top - rect.y) / rect.height * 2 + 1)
    ...
    const resize = new ResizeObserver(() => renderer.setSize(el.clientWidth, el.clientHeight))
    resize.observe(el)
    ...
    function render(time: number) {
      ...
      if (resetVersion !== state.cameraReset) {
        ...
        resetCamera()
        for (const c of [top, front, left]) { c.zoom = 1; c.updateProjectionMatrix() }
      }
      controls.update()
      for (let i = 0; i < state.project.objects.length; i++) {
        const object = state.project.objects[i], solid = models.children[i], wire = wireModels.children[i]
        if (solid && wire) {
      ...
      sun.intensity = state.lighting === 'Studio' ? 4.1 : 5.6
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const r = getRect(i), camera = cameras[i]
  ...
  useEffect(() => {
    const models = sceneRef.current, wires = wiresRef.current
    ...
    for (const object of props.project.objects) {
      ...
      model.traverse(child => {
        ...
        const geometry = child.geometry.type === 'TorusGeometry' || child.geometry.type === 'TubeGeometry' || child.geometry.type === 'SphereGeometry'
          ? new THREE.WireframeGeometry(child.geometry) : new THREE.EdgesGeometry(child.geometry, 12)
        const color = object.category === 'Sculptures' ? '#bcab80' : object.category === 'Landscape' ? '#7d9a68' : '#819b99'
        const line = new THREE.LineSegments(geometry, new THREE.LineBasicMaterial({ color, transparent: true, opacity: object.id === props.selected ? 1 : 0.67 }))
  ...
  return <div className={`viewport-host ${props.maximized ? 'maximized' : ''}`} ref={host}>
    {error && <div className="webgl-error" role="alert">{error}</div>}
    {views.map((view, i) => <div key={view} className={`view-overlay view-${i} ${view === props.active ? 'active' : ''} ${props.maximized && view !== props.active ? 'hidden' : ''}`}>
      <div className="view-label"><span>[+]</span><b>{view}</b><span>[Standard]</span><span>[{i === 3 ? 'Default Shading' : 'Wireframe'}]</span></div>
      <button className="view-max" title={`${props.maximized ? 'Restore four viewports' : `Maximize ${view}`} (Alt+W)`} aria-label={`${props.maximized ? 'Restore four viewports' : `Maximize ${view}`}`} onClick={() => { props.onActive(view); props.onMaximize() }}>{props.maximized ? <Minimize2 size={12} /> : <Maximize2 size={12} />}</button>
      <div className="view-cube"><span>{i === 3 ? 'FRONT' : view.toUpperCase()}</span><i>N</i></div>
      <div className="axis-indicator"><span className="axis-y">Y</span><span className="axis-z">Z</span><span className="axis-x">X</span></div>
      {i === 3 && <div className="scene-caption"><b>FORMA</b><span>GALLERY / STUDY 01</span></div>}
