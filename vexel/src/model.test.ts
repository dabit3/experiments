...
describe('animation editing and sampling', () => {
  it('keeps 24 FPS timeline time across skipped render frames and loop boundaries', () => {
    expect(playbackFrame(0, 500)).toBe(12)
    expect(playbackFrame(0, 8000)).toBe(91)
    expect(playbackFrame(98, 250)).toBe(3)
    expect(playbackFrame(50, 0)).toBe(50)
    expect(playbackFrame(50, 1000)).toBe(74)
  })
...
describe('real geometry and OBJ export', () => {
  ...
  it('exports baked modifiers and samples the requested animation frame', () => {
    ...
    object.twist = 90
    object.keys = [
      { ...transform(object), frame: 0 },
      { ...transform(object), position: [12, 1, 2.8], frame: 100 },
    ]
