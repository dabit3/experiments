...
export function buildObject(object: SceneObject): THREE.Group {
  ...
  const box = (size: [number, number, number], pos: [number, number, number], mat = base) =>
    mesh(new THREE.BoxGeometry(...size, 1, object.twist ? 40 : 1, 1), mat, pos)
  switch (object.kind) {
    case 'box':
      box([1, 1, 1], [0, 0, 0])
      break
    case 'sphere':
      mesh(new THREE.SphereGeometry(0.65, 32, 24))
      break
    case 'cylinder':
      mesh(new THREE.CylinderGeometry(0.65, 0.65, 1.8, 32, 24))
      break
    case 'torus':
      mesh(new THREE.TorusGeometry(0.85, 0.2, 16, 56))
      break
    case 'plinth':
      ...
      const curve = new THREE.CatmullRomCurve3(
        Array.from({ length: 121 }, (_, i) => {
          ...
          return new THREE.Vector3(
            Math.sin(a) * (1.0 + 0.18 * Math.cos(a * 3)),
            1.35 + Math.cos(a) * 1.2,
            Math.sin(a * 2) * 0.43,
          )
        }),
    ...
    case 'rib': {
      ...
      for (let i = 60; i >= 0; i--) {
        ...
        shape.lineTo(6.38 * Math.cos(a), 2.25 + 3.95 * Math.sin(a))
      }
      shape.lineTo(6.38, 0)
      shape.closePath()
      mesh(
    ...
    case 'shell': {
      ...
      disc.scale.y = 1.3
      box([2, 0.022, 0.024], [-3.6, 1.35, -9.9], dark)
    ...
    case 'bench': {
      ...
      for (const x of [-0.43, 0.43])
        for (const z of [-1.15, 1.15]) box([0.08, 0.57, 0.08], [x, 0.29, z], material('#35332e'))
    ...
    case 'plant': {
      ...
      for (let i = 0; i < 36; i++) {
        const a = i * 2.39996,
          radius = 0.3 + (i % 5) * 0.12
        const leaf = mesh(new THREE.SphereGeometry(1, 8, 6), i % 3 === 0 ? material('#68784c') : base, [
          Math.cos(a) * radius,
          1.65 + (i % 7) * 0.15,
          Math.sin(a) * radius,
        ])
