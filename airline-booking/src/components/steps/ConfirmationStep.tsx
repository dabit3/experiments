...
import type { Booking, Contact, Extras, FlightSelection, Leg, Passenger, SearchParams, SeatAssignments } from '../../types'
...
export function ConfirmationStep({ search, selections, passengers, contact, seats, extras, booking, onRestart }: Props) {
        ...
        <div className="confirm-actions">
          <button type="button" className="btn btn-primary" onClick={downloadIcs}>
            <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden>
              <path d="M12 4v11m0 0l-4-4m4 4l4-4M5 19h14" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            Download .ics
          </button>
        ...
        return (
          <section key={leg} className="pass-group">
            <header className="pass-group-head">
              <h3>
                {leg === 'outbound' ? 'Outbound' : 'Return'} · {AIRPORT_BY_CODE[flight.from].city} → {AIRPORT_BY_CODE[flight.to].city}
              </h3>
              <span className="muted">
                {formatLong(date)} · {to12h(flight.depart)} – {to12h(times.arriveLocal)}
                {times.dayOffset > 0 ? ` (+${times.dayOffset})` : ''} · {flight.stops === 0 ? 'Nonstop' : `${flight.stops} stop`}
              </span>
            </header>
