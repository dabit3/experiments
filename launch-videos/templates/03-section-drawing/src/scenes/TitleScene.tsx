...
export const TitleScene: React.FC<{ props: LaunchProps; durationInFrames: number }> = ({
  ...
  const headTop = 396;
  const headlineWidth = 1180;

  return (
    <AbsoluteFill style={{ backgroundColor: brand.paper }}>
      <Grid brand={brand} opacity={1} />
      <SheetMarks brand={brand} opacity={t0} />
      <div style={{ position: "absolute", left: MARGIN, top: MARGIN - 2, opacity: t0 }}>
        <Logo brand={brand} height={30} />
      </div>
      <div style={{ position: "absolute", left: MARGIN, top: headTop - 52, opacity: t0 * (1 - out) }}>
        <Eyebrow brand={brand}>
      ...
      <div style={{ position: "absolute", left: MARGIN, top: headTop, width: headlineWidth, opacity: 1 - out }}>
        <MaskReveal progress={tHead}>
          <Headline brand={brand} text={content.headline} accent={content.headlineAccent} size={92} />
        </MaskReveal>
      </div>
      <DimensionLine
        brand={brand}
      ...
      >
        <Body brand={brand} size={30} color={brand.inkMuted}>
          {content.subhead}
        </Body>
      </div>
      <div
