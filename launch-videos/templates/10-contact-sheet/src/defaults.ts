import type { LaunchProps } from "./schema";

export const FPS = 30;
export const WIDTH = 1920;
export const HEIGHT = 1080;

export const defaultBrand: LaunchProps["brand"] = {
  paper: "#F7F6F5",
  surface: "#EFEFEF",
  surfaceAlt: "#FCFCFC",
  line: "#E7E7E7",
  ink: "#191919",
  inkMuted: "#7D7D7D",
  inkSubtle: "#919191",
  accent: "#2200FF",
  black: "#141414",
  blackRaised: "#1F1F1F",
  white: "#FFFFFF",
  fontFamily: '"NB International Pro", "Inter", "Helvetica Neue", Arial, sans-serif',
  monoFontFamily: '"Geist Mono", ui-monospace, SFMono-Regular, Menlo, monospace',
  logoLight: "logos/devin-lockup-horizontal-black.png",
  logoDark: "logos/devin-lockup-horizontal-white.png",
};

export const defaultContent: LaunchProps["content"] = {
  featureName: "Devin on Mac",
  eyebrow: "New",
  headline: "Devin now runs in a Mac VM",
  headlineAccentWord: "Mac",
  subhead:
    "Build, run, and test Mac and iOS apps in a cloud Mac — with a live iPhone Simulator in your session.",
  captions: [
    "Pick macOS when you start a session — same price as Linux.",
    "Devin builds the app in Xcode and runs the full test suite.",
    "It taps, types, and scrolls through the app like a person.",
    "Watch it live in the iPhone Simulator tab — and tap in yourself.",
    "Reproduce a bug, fix it, and prove the fix on screen.",
    "Check iPhone and iPad sizes, dark mode, and orientations.",
    "Ship with a PR that shows the app working — not a 20-minute CI wait.",
  ],
  useCases: [
    "Build & test a feature",
    "Reproduce & fix a bug",
    "QA before shipping",
    "Screens, sizes & dark mode",
    "Upgrade Swift & dependencies",
  ],
  stages: ["Request", "Build", "Run & test", "Verify", "PR"],
  cta: { label: "Start a Mac session", url: "app.devin.ai" },
  outroLine: "The only coding agent with a cloud Mac.",
  speedBadge: "3x",
};

export const defaultMedia: LaunchProps["media"] = {
  pick: { src: "screenshots/devin-web-4.png", kind: "image", fit: "contain", sourceWidth: 2988, sourceHeight: 1622 },
  work: { src: "recordings/devin-working-4.mp4", kind: "video", startFrom: 90, thumbFrame: 106 },
  simulator: {
    src: "recordings/androidios.mp4",
    kind: "video",
    startFrom: 60,
    thumbFrame: 300,
    sourceWidth: 1920,
    sourceHeight: 1148,
    // iPhone Simulator window only; the Android emulator on the right is cropped out.
    crop: { x: 0.165, y: 0.155, w: 0.185, h: 0.62 },
    fit: "contain",
  },
  verify: { src: "screenshots/devin-web-18.png", kind: "image", fit: "contain", sourceWidth: 2978, sourceHeight: 1626 },
  result: { src: "screenshots/devin-web-12.png", kind: "image", fit: "contain", sourceWidth: 2978, sourceHeight: 1622 },
};

export const defaultProps: LaunchProps = {
  brand: defaultBrand,
  content: defaultContent,
  media: defaultMedia,
  sheet: { columns: 5, cellWidth: 326, gap: 24, top: 456, metadata: "Contact sheet · 5 frames · Devin on Mac" },
  scenes: [
    { id: "sheet", kind: "sheet", durationInFrames: 120, selectAtFrame: 72 },
    {
      id: "pick",
      kind: "clip",
      durationInFrames: 180,
      frameIndex: 0,
      media: "pick",
      caption: defaultContent.captions[0],
      label: defaultContent.stages[0],
      expandFrames: 14,
      freezeHoldFrames: 0,
      indexFrames: 30,
    },
    {
      id: "build",
      kind: "clip",
      durationInFrames: 240,
      frameIndex: 1,
      media: "work",
      caption: defaultContent.captions[1],
      label: defaultContent.stages[1],
      expandFrames: 14,
      freezeHoldFrames: 45,
      indexFrames: 30,
    },
    {
      id: "tap",
      kind: "clip",
      durationInFrames: 240,
      frameIndex: 2,
      media: "simulator",
      caption: defaultContent.captions[2],
      label: defaultContent.stages[2],
      expandFrames: 14,
      freezeHoldFrames: 45,
      indexFrames: 30,
    },
    {
      id: "verify",
      kind: "clip",
      durationInFrames: 210,
      frameIndex: 3,
      media: "verify",
      caption: defaultContent.captions[3],
      label: defaultContent.stages[3],
      expandFrames: 14,
      freezeHoldFrames: 0,
      indexFrames: 30,
    },
    {
      id: "ship",
      kind: "clip",
      durationInFrames: 210,
      frameIndex: 4,
      media: "result",
      caption: defaultContent.captions[6],
      label: defaultContent.stages[4],
      expandFrames: 14,
      freezeHoldFrames: 0,
      indexFrames: 30,
    },
    { id: "outro", kind: "outro", durationInFrames: 150, frameIndex: 4, media: "result", expandFrames: 18 },
  ],
};
