...
| feature-dash | Dash for speed | `simulation.dart` dash impulse; F/L / Dash button | core determinism test uses dash in bot plan |
...
| ID | Journey | Test |
| --- | --- | --- |
| journey-four-platform-match | web + iOS Simulator + Android + macOS join `E2E4`, lobby → countdown → gameplay → results; final score/stars/served/tips/expired/bestCombo/wrongServes/burnt identical on all four clients, the server and the offline plan | `evidence/e2e/<final>/summary.json`, `e2e.log`, screenshots, `four-way-match.mov`; natural overtime separately in `evidence/tests/arcade-ui/report.md` |
...
`evidence/diffs/<state>-<platform>{,.normalized,.core,.density}.png`.

The final arcade matrix keeps the existing placement/core limits and adds
an unshifted 8-logical-pixel colour-density pass at 32/255 channel tolerance.
All nine pairs have zero differing 2-pixel blocks and zero surviving core
pixels. The density pass has zero differing cells except one macOS results
cell (within the documented limit of two). All three passes must pass.
The self-test catches erased 16/24-pixel elements, 3/4-pixel moves and
recolouring in every captured state; the density pass catches small glyph
movements that the previous placement/core combination missed.
...
| asset-arcade-key-art | Original toy-diorama home illustration | generated specifically for Panic Pantry without a source-game image, bundled as `app/assets/arcade-kitchen.png`; clean builds, smoke home captures and normalized native comparisons |
...
| integration-review-video | Programmatic edited review reel cut from the E2E and browser-smoke evidence (title/chapter cards, labelled four-platform boards, log-derived captions, pass cards, results table, verdict), produced for passing and failing runs | `test/review-video/build.mjs` via `test/review-video.sh`, invoked by `multiplayer-e2e.sh`; `review-video.{mp4,gif,json,log}` next to the E2E run; `browser-smoke.webm` from `test/ui/smoke.mjs` |
