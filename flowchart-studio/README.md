...
- The document autosaves to `localStorage`; there is no backend and no network access at runtime.
...
|---|------|-----------------|
| 1 | Maximise Chrome, open `http://localhost:5173`, click **New** | Empty dotted canvas, palette on the left, minimap bottom-right, status bar reads `0 nodes · 0 edges`. |
| 2 | Drag seven shapes from the palette onto the canvas: Start pill, Process ×4, Decision diamond, End pill | Each drop creates a node snapped to the grid; the status bar counts up to `7 nodes`. |
...
| 6 | Marquee-select three nodes by dragging a rectangle over empty canvas around them, then drag one of them | The three nodes highlight together, the status bar says `3 nodes + 2 edges selected` (the edges between them are picked up too), and all three move as a group. |
...
| 9 | Scroll the wheel down over the canvas, then back up | The zoom percentage in the toolbar drops and rises again; the minimap viewport rectangle grows and shrinks. |
