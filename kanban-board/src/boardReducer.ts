...
export type BoardAction =
  ...
  | { type: 'reorder-card'; columnId: ColumnId; fromIndex: number; toIndex: number }
  | { type: 'add-card'; columnId: ColumnId; title: string }
  | { type: 'update-card'; cardId: string; patch: Partial<Omit<Card, 'id'>> }
  | { type: 'delete-card'; cardId: string }
  | { type: 'reset' }

export function findColumnOfCard(board: BoardState, cardId: string): ColumnId | undefined {
      ...
      return {
        ...
        columns: board.columns.map((column) => {
          if (column.id === fromColumn && column.id === action.toColumn) {
            ...
            return { ...column, cardIds: arrayMove(column.cardIds, fromIndex, action.toIndex) }
          }
          if (column.id === fromColumn) {
            return { ...column, cardIds: column.cardIds.filter((id) => id !== action.cardId) }
          }
      ...
      return {
        ...
        columns: board.columns.map((column) =>
          column.id === action.columnId
            ? { ...column, cardIds: arrayMove(column.cardIds, action.fromIndex, action.toIndex) }
            : column,
        ),
      }
    case 'add-card': {
      ...
      const card: Card = { id, title, description: '', labels: [], assigneeId: null }
      return {
        ...board,
        nextCardNumber: board.nextCardNumber + 1,
        cards: { ...board.cards, [id]: card },
        columns: board.columns.map((column) =>
          column.id === action.columnId ? { ...column, cardIds: [...column.cardIds, id] } : column,
        ),
      }
    }
    case 'update-card': {
      ...
      return { ...board, cards: { ...board.cards, [action.cardId]: { ...existing, ...action.patch } } }
    }
    case 'delete-card': {
