    ...
    TimelineView(.animation(minimumInterval: 1.0 / 15, paused: !animated || reduceMotion)) {
      ...
      Canvas { context, size in
        ...
        context.fill(
          ...
          with: .linearGradient(
            Gradient(colors: [Color(red: 0.09, green: 0.09, blue: 0.20), HushStyle.ink]),
            startPoint: .zero, endPoint: CGPoint(x: 0, y: h)))
        let moon = CGPoint(x: w * 0.7, y: h * 0.23)
        ...
        context.fill(
          Path(ellipseIn: CGRect(x: moon.x - 11, y: moon.y - 27, width: 38, height: 38)),
        ...
        for line in 0..<27 {
          ...
          for step in 0...80 {
            ...
            let y = baseline + ripple * (3 + depth * 9) * (0.4 + ocean)
            if step == 0 {
