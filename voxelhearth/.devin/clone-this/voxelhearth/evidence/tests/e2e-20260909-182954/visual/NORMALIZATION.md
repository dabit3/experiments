...
The asserted comparison is structural:
...
3. `*.layout.raw.diff.png` records the pre-snap wireframe diff (34 px lobby,
   203 px results: sub-2-px glyph-advance differences between text engines).
...
the local player name, which differ per client (62/69 nodes matched).
iOS (402×874) renders the phone layout family and is recorded, not compared.
