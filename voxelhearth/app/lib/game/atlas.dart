...
class Tiles {
  static const stone = 0, dirt = 1, grassTop = 2, grassSide = 3, sand = 4, water = 5, logSide = 6, logTop = 7;
  ...
  static const torch = 16,
      ...
      chestTop = 22,
      chestFront = 23;
  static const bedTop = 24, bedSide = 25, glass = 26, gravel = 27, snow = 28, flower = 29, tallGrass = 30, brick = 31;
  static const stoneBrick = 32,
      ...
      frostLogSide = 38,
      frostLogTop = 39;
  static const frostLeaves = 40, clay = 41, snowGrassSide = 42, kilnSide = 43, lanternTop = 44, bedBottom = 45;
  static const stick = 48, coal = 49, ironIngot = 50, goldIngot = 51, emberGem = 52, rawIron = 53, rawGold = 54;
...
class TileFlag {
  static const solid = 0, cutout = 1, fluid = 2, glass = 3, torch = 4, plant = 5, cactus = 6, bed = 7, lantern = 8;
}
...
BlockTiles tilesFor(int blockId) {
  switch (blockId) {
    ...
    case Ids.water:
      return const BlockTiles(Tiles.water, Tiles.water, Tiles.water, TileFlag.fluid);
    case Ids.log:
      return const BlockTiles(Tiles.logTop, Tiles.logSide, Tiles.logTop, 0);
    case Ids.leaves:
      return const BlockTiles(Tiles.leaves, Tiles.leaves, Tiles.leaves, TileFlag.cutout);
    case Ids.planks:
    ...
    case Ids.emberOre:
      return const BlockTiles(Tiles.emberOre, Tiles.emberOre, Tiles.emberOre, 0);
    case Ids.bedrock:
      return const BlockTiles(Tiles.bedrock, Tiles.bedrock, Tiles.bedrock, 0);
    case Ids.torch:
      return const BlockTiles(Tiles.torch, Tiles.torch, Tiles.torch, TileFlag.torch);
    case Ids.workbench:
    ...
    case Ids.chest:
      return const BlockTiles(Tiles.chestTop, Tiles.chestFront, Tiles.chestTop, 0);
    case Ids.bed:
      return const BlockTiles(Tiles.bedTop, Tiles.bedSide, Tiles.bedBottom, TileFlag.bed);
    case Ids.glass:
      return const BlockTiles(Tiles.glass, Tiles.glass, Tiles.glass, TileFlag.glass);
    case Ids.gravel:
    ...
    case Ids.flower:
      return const BlockTiles(Tiles.flower, Tiles.flower, Tiles.flower, TileFlag.plant);
    case Ids.tallGrass:
      return const BlockTiles(Tiles.tallGrass, Tiles.tallGrass, Tiles.tallGrass, TileFlag.plant);
    case Ids.brick:
      return const BlockTiles(Tiles.brick, Tiles.brick, Tiles.brick, 0);
    case Ids.stoneBrick:
      return const BlockTiles(Tiles.stoneBrick, Tiles.stoneBrick, Tiles.stoneBrick, 0);
    case Ids.lantern:
      return const BlockTiles(Tiles.lanternTop, Tiles.lantern, Tiles.lanternTop, TileFlag.lantern);
    case Ids.cactus:
      return const BlockTiles(Tiles.cactusTop, Tiles.cactusSide, Tiles.cactusTop, TileFlag.cactus);
    case Ids.wool:
      return const BlockTiles(Tiles.wool, Tiles.wool, Tiles.wool, 0);
    case Ids.mossStone:
      return const BlockTiles(Tiles.mossStone, Tiles.mossStone, Tiles.mossStone, 0);
    case Ids.frostLog:
      return const BlockTiles(Tiles.frostLogTop, Tiles.frostLogSide, Tiles.frostLogTop, 0);
    case Ids.frostLeaves:
      return const BlockTiles(Tiles.frostLeaves, Tiles.frostLeaves, Tiles.frostLeaves, TileFlag.cutout);
    case Ids.clay:
...
class _Px {
  ...
  void cells(int base, int line, {int cellW = 8, int cellH = 4, int offset = 4}) {
    ...
    for (var y = 0; y < 16; y++) {
      ...
      for (var x = 0; x < 16; x++) {
        ...
        set(x, y, inLineY || inLineX ? line : shade(base, 0.9 + rng.nextDouble() * 0.2));
      }
    }
  }
}
...
int shade(int argb, double f) {
  ...
  return (ch((argb >> 16) & 0xff) << 16) | (ch((argb >> 8) & 0xff) << 8) | ch(argb & 0xff);
}
...
class AtlasData {
  ...
  int pixel(int tile, int x, int y) {
    ...
    return (pixels[o + 3] << 24) | (pixels[o] << 16) | (pixels[o + 1] << 8) | pixels[o + 2];
  }
  ...
  Future<ui.Image> toImage() {
    ...
    ui.decodeImageFromPixels(pixels, Tiles.width, Tiles.height, ui.PixelFormat.rgba8888, c.complete);
    return c.future;
  }
  ...
  Future<ui.Image> tileMapImage() {
    final buf = Uint8List(256 * 2 * 4);
    for (var id = 0; id <= Ids.lastBlock; id++) {
      final t = tilesFor(id);
      buf[id * 4] = t.top;
      buf[id * 4 + 1] = t.side;
      buf[id * 4 + 2] = t.bottom;
    ...
    final c = Completer<ui.Image>();
    ui.decodeImageFromPixels(buf, 256, 2, ui.PixelFormat.rgba8888, c.complete);
    return c.future;
  }
}
...
AtlasData buildAtlas() {
  ...
  px(Tiles.sand).noise(0xe0cf94, 0.07, blend: 0xd2bf80, blendChance: 0.2);
  {
    ...
    for (var i = 0; i < 6; i++) {
      final y = p.rng.nextInt(16), x = p.rng.nextInt(16), len = 3 + p.rng.nextInt(4);
      for (var k = 0; k < len; k++) {
  ...
  }
  {
    ...
    for (var r = 1; r < 8; r += 2) {
      for (var a = 0; a < 64; a++) {
        final x = (8 + r * _cos(a / 64)).round(), y = (8 + r * _sin(a / 64)).round();
    ...
    for (var r = 2; r < 8; r += 2) {
      for (var a = 0; a < 90; a++) {
        final x = (7.5 + r * _cos(a / 90)).round(), y = (7.5 + r * _sin(a / 90)).round();
  ...
  }
  {
    ...
    for (var i = 0; i < 9; i++) {
      final x = (i % 3) * 5 + p.rng.nextInt(2), y = (i ~/ 3) * 5 + p.rng.nextInt(2);
  ...
  }
  {
    ...
    for (var y = 0; y < 16; y++) {
      for (var x = 0; x < 16; x++) {
        ...
        p.set(x, y, snowy ? 0xe4eef4 : shade(0x3a6e5a, 0.75 + p.rng.nextDouble() * 0.5));
      }
    }
  }
  {
  ...
  }
  {
    ...
    for (var y = 0; y < 16; y++) {
      for (var x = 0; x < 16; x++) {
        p.set(x, y, shade(y < 3 ? 0x6f4d2c : 0xb98a56, 0.93 + p.rng.nextDouble() * 0.14));
      }
    }
    ...
    for (var y = 0; y < 16; y++) {
      for (var x = 0; x < 16; x++) {
        p.set(
          x,
          y,
          y < 8 ? shade(0xb83a3a, 0.95 + p.rng.nextDouble() * 0.1) : shade(0x8a6a3a, 0.95 + p.rng.nextDouble() * 0.1),
        );
      }
    }
  ...
  const woodHead = 0xb98a56, stoneHead = 0x8a8d90, ironHead = 0xe1e4e8, emberHead = 0xff7a1f, handleC = 0x8a6a3a;
