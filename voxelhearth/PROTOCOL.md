...
VM (server, iOS, Android, macOS) and dart2js (web) agree bit-for-bit.
...
Drive actions implemented by every client (see `app/lib/shell.dart` and
`app/lib/game/game_controller.dart`): `screen`, `wait_screen`, `create_room`,
...
intent extras (Android). `VH_NAME`, `VH_SERVER`, `VH_JOIN` and `VH_CREATE`
work the same way.
