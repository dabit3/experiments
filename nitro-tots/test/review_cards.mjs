...
const fontUrl = (name) => `data:font/ttf;base64,${readFileSync(resolve(fonts, name)).toString('base64')}`;
...
body { font-family: Nunito, sans-serif; color: #241E3A; -webkit-font-smoothing: antialiased; }
.card { position: relative; width: 100%; height: 100%; box-sizing: border-box;
  background: linear-gradient(135deg, #FFF8F0 0%, #FBE3EC 55%, #FFE9D6 100%); }
.transparent { background: transparent; }
.confetti { position: absolute; border-radius: 40%; opacity: .55; }
...
.title { font-family: Fredoka; font-weight: 700; font-size: 84px; line-height: 1.02; color: #241E3A; margin: 10px 0 8px; }
.subtitle { font-size: 30px; font-weight: 700; color: #6E6689; }
.lines { margin-top: 34px; display: grid; gap: 10px; font-size: 24px; font-weight: 700; color: #3B3355; }
...
.pill { background: rgba(36,30,58,.88); color: #fff; border-radius: 18px; padding: 16px 26px; box-shadow: 0 6px 0 rgba(0,0,0,.18); max-width: 900px; }
...
.notice { background: #16122A; color: #fff; }
...
.ok { color: #2E9E4F; } .warn { color: #D94E1F; } .info { color: #1C86D6; }
